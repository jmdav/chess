...
public class StandardAPITests {
  ...
  public void registerSuccess() {
    ...
    Assertions.assertEquals(
        newUser.getUsername(), registerResult.getUsername(),
  ...
  public void registerBadRequest() {
    ...
    TestUser registerRequest =
        new TestUser(newUser.getUsername(), null, newUser.getEmail());
  ...
  public void createGameSuccess() {
    ...
    Assertions.assertNotNull(createResult.getGameID(),
                             "Result did not return a game ID");
    Assertions.assertTrue(createResult.getGameID() > 0,
                          "Result returned invalid game ID");
  }
  ...
  public void createGameBadRequest() {
    TestCreateResult createResult =
        serverFacade.createGame(new TestCreateRequest(null), existingAuth);
  ...
  public void joinGameSuccess() {
    ...
    // join as white
    TestJoinRequest joinRequest = new TestJoinRequest(ChessGame.TeamColor.WHITE,
    ...
    Assertions.assertNotNull(listResult.getGames(),
                             "List result did not contain games");
    Assertions.assertEquals(1, listResult.getGames().length,
                            "List result is incorrect size");
    Assertions.assertEquals(
        existingUser.getUsername(), listResult.getGames()[0].getWhiteUsername(),
  ...
  public void joinGameUnauthorized() {
    ...
    TestResult joinResult =
        serverFacade.joinPlayer(joinRequest, existingAuth + "bad stuff");
  ...
  public void joinGameBadColor() {
    ...
    for (String color : new String[] {null, "", "GREEN"}) {
      assertHttpBadRequest(serverFacade.joinPlayer(
          new TestJoinRequest(color, gameID), existingAuth));
    }
  }
  ...
  public void joinGameStealColor() {
    ...
    TestJoinRequest joinRequest = new TestJoinRequest(ChessGame.TeamColor.BLACK,
                                                      createResult.getGameID());
  ...
  public void joinGameBadGameId() {
    ...
    // try join as white
    TestJoinRequest joinRequest =
        new TestJoinRequest(ChessGame.TeamColor.WHITE, null);
    TestResult joinResult = serverFacade.joinPlayer(joinRequest, existingAuth);
  ...
  public void listGamesEmpty() {
    ...
    assertHttpOk(result);
    Assertions.assertNotNull(result.getGames(),
                             "List result did not contain an empty game list");
    Assertions.assertEquals(0, result.getGames().length,
  ...
  public void listGamesSuccess() {
    ...
    TestCreateResult game1 = serverFacade.createGame(
        new TestCreateRequest(game1Name), authA.getAuthToken());
    serverFacade.joinPlayer(
        new TestJoinRequest(ChessGame.TeamColor.BLACK, game1.getGameID()),
        authA.getAuthToken());
    expectedList[0] = new TestListEntry(game1.getGameID(), game1Name, null,
                                        authA.getUsername());

    // 1 as white from B
    String game2Name = "Lonely";
    TestCreateResult game2 = serverFacade.createGame(
        new TestCreateRequest(game2Name), authB.getAuthToken());
    serverFacade.joinPlayer(
        new TestJoinRequest(ChessGame.TeamColor.WHITE, game2.getGameID()),
        authB.getAuthToken());
    expectedList[1] = new TestListEntry(game2.getGameID(), game2Name,
                                        authB.getUsername(), null);

    // 1 of each from C
    String game3Name = "GG";
    TestCreateResult game3 = serverFacade.createGame(
        new TestCreateRequest(game3Name), authC.getAuthToken());
    serverFacade.joinPlayer(
    ...
    serverFacade.joinPlayer(
        ...
        authA.getAuthToken());
    expectedList[2] = new TestListEntry(
        game3.getGameID(), game3Name, authC.getUsername(), authA.getUsername());

    // C play self
    String game4Name = "All by myself";
    TestCreateResult game4 = serverFacade.createGame(
        new TestCreateRequest(game4Name), authC.getAuthToken());
    serverFacade.joinPlayer(
    ...
    serverFacade.joinPlayer(
        ...
        authC.getAuthToken());
    expectedList[3] = new TestListEntry(
        game4.getGameID(), game4Name, authC.getUsername(), authC.getUsername());
    ...
    Assertions.assertNotNull(returnedList,
                             "List result did not contain a list of games");
    Comparator<TestListEntry> gameIdComparator =
        Comparator.comparingInt(TestListEntry::getGameID);
    Arrays.sort(expectedList, gameIdComparator);
    Arrays.sort(returnedList, gameIdComparator);

    // check
    Assertions.assertArrayEquals(expectedList, returnedList,
                                 "Returned Games list was incorrect");
  }
  ...
  public void uniqueAuthorizationTokens() {
    ...
    Assertions.assertNotNull(loginOne.getAuthToken(),
                             "Login result did not contain an authToken");

    TestAuthResult loginTwo = serverFacade.login(existingUser);
    assertHttpOk(loginTwo);
    Assertions.assertNotNull(loginTwo.getAuthToken(),
                             "Login result did not contain an authToken");

    Assertions.assertNotEquals(
        existingAuth, loginOne.getAuthToken(),
        "Authtoken returned by login matched authtoken from prior register");
    Assertions.assertNotEquals(
        existingAuth, loginTwo.getAuthToken(),
        "Authtoken returned by login matched authtoken from prior register");
    Assertions.assertNotEquals(
        loginOne.getAuthToken(), loginTwo.getAuthToken(),
    ...
    TestJoinRequest joinRequest = new TestJoinRequest(ChessGame.TeamColor.WHITE,
                                                      createResult.getGameID());
    TestResult joinResult =
        serverFacade.joinPlayer(joinRequest, loginOne.getAuthToken());
    assertHttpOk(joinResult);
  ...
  public void clearData() {
    // create filler games
    serverFacade.createGame(new TestCreateRequest("Mediocre game"),
                            existingAuth);
    serverFacade.createGame(new TestCreateRequest("Awesome game"),
                            existingAuth);
    ...
    TestCreateResult createResult = serverFacade.createGame(
        new TestCreateRequest("Clear game"), registerResult.getAuthToken());
    ...
    Assertions.assertNotNull(
        listResult.getGames(),
        "List result did not contain an empty list of games");
  ...
  private void assertHttpOk(TestResult result) {
    Assertions.assertEquals(
        HttpURLConnection.HTTP_OK, serverFacade.getStatusCode(),
        "Server response code was not 200 OK (message: %s)".formatted(
            result.getMessage()));
  ...
  private void assertHttpUnauthorized(TestResult result) {
    assertHttpError(result, HttpURLConnection.HTTP_UNAUTHORIZED,
                    "Unauthorized");
  }
  ...
  private void assertHttpError(TestResult result, int statusCode,
                               String message) {
    Assertions.assertEquals(
        statusCode, serverFacade.getStatusCode(),
        "Server response code was not %d %s (message: %s)".formatted(
            statusCode, message, result.getMessage()));
