...
public class InputHandler {
  ...
  public HandlerResponse parseSignedIn(SessionData data, String in)
    ...
    case "joingame":
      ...
      Integer newGameID = listToGameID(tokens[1]);
      ...
      server.joinGame(data, newGameID, color);
      socket.join(data.authToken(), newGameID);
  ...
  public HandlerResponse parseInGame(SessionData data, String in)
    ...
    case "move":
      if (tokens.length < 3) {
        throw new ResponseException(
            "Error: insufficient arguments. Expected <start position> <end "
            + "position> <piece promotion (Q, R, B, N)(if applicable)>");
      }
      ...
      if (tokens.length == 4) {
        try {
          ...
          throw new ResponseException(
              "Error: invalid piece promotion. Expected (Q, R, B, N)");
        }
      } else {
        move = new chess.ChessMove(parsePosition(tokens[1]),
